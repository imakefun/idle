...
function Tradeskills() {
  ...
  const handleDelete = async (row) => {
    if (!confirm(`Are you sure you want to delete tradeskill "${row.name}"?`)) {
      return;
    }
    ...
    try {
      ...
      alert(`Successfully deleted tradeskill "${row.name}"`);
    } catch (error) {
  ...
  const handleSave = async (formData) => {
    try {
      if (editingRow) {
        ...
        alert(`Successfully updated tradeskill "${formData.name}"`);
      } else {
        await createRow(sheetName, formData);
        alert(`Successfully created tradeskill "${formData.name}"`);
      }
    } catch (error) {
      throw new Error(error.message || 'Failed to save tradeskill');
    }
  };
  ...
  if (isLoading && !sheetData) {
    return (
      <div>
        <h1>🛠️ Tradeskills</h1>
        <p className="text-secondary">Loading tradeskills...</p>
      </div>
    );
  }
  ...
  if (!sheetData) {
    return (
      <div>
        <h1>🛠️ Tradeskills</h1>
        <p className="text-error">Failed to load tradeskills data</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="page-header" style={{ marginBottom: '1.5rem' }}>
        <div>
          <h1>🛠️ Tradeskills</h1>
          <p className="text-secondary">Manage crafting professions</p>
          {!isAuthenticated && (
      ...
      <RowEditor
        ...
        row={editingRow}
        headers={sheetData.headers}
        title={editingRow ? `Edit Tradeskill: ${editingRow.name}` : 'Create New Tradeskill'}
